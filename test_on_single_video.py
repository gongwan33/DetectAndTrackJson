...
def main(name_scope, gpu_dev, num_images, args):
    ...
    jsonOutput = JSONOutput('resvideo/' + args.vid_name + '.json', conf)
    ...
    if '2d_best' in args.cfg_file:
        for i in range(num_images):
            ...
            jsonOutput.write(cls_boxes_i, cls_keyps_i, i + 1)
    ...
    cfg.TRACKING.DEBUG.UPPER_BOUND_5_GT_KPS_ONLY = True
    cfg.TRACKING.DEBUG.UPPER_BOUND_4_EVAL_UPPER_BOUND = True
    ...
    print("Box confidence threshold: %f"%conf)

    dets = _prune_bad_detections(dets, conf)

    # lstm model should not be opened. They are dumped code. https://github.com/facebookresearch/DetectAndTrack/issues/13
    # cfg.TRACKING.LSTM_TEST.LSTM_TRACKING_ON = True
    ...
    print("Frame rate: %f"%(num_images/total_time))
